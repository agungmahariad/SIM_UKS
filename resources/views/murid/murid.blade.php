			...
			<div class="col-lg-12">
				<div class="panel panel-default">
					<div class="panel-heading">
						<a href="{{ url('createmurid') }}" class="btn btn-primary">Tambah Siswa</a>
					</div>
								...
								@if (count($data))
									@foreach($data as $row)
										<tr>
											...
											<td>{{ $row->rayon }}</td>
											<td align="center">
												<a href="{{ url('editmurid/'.$row->nis) }}" class="btn btn-success">Ubah</a>
												<a href="{{ url('deletemurid/'.$row->nis) }}" class="btn btn-danger" onClick="return confirm('Hapus ?')">Hapus</a>
											</td>
										</tr>
									@endforeach
								@else
